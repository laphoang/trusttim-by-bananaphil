...
| `Pitch-Deck-Content.md` | Slide-by-slide narration content (English, ~5 min, 8 slides) for the pitch deck — wow-first (emergency-safety hook), each slide tagged with the rubric criteria it targets, doctor as lead voice on pain/business/close. Includes a 4-min live-pitch trim and a Q&A-by-judge-archetype note. Grounded in the scoring rubric + Winning Playbook pitch arc. |
| `Live-Demo-Script.md` | The ~5-min live demo video script (Vietnamese) — wow-first order (emergency → same-keyword-normal contrast → grounded cited table → multi-intent → out-of-scope → booking → close), each beat with the exact query to type and what to point at on-screen, plus demo-prep tips. Uses the real working app's example queries. |
...
- **...get the slide-by-slide pitch deck content (English) or the Vietnamese live-demo script, both wow-first and rubric-optimized** → `hackathon_docs/guide/Pitch-Deck-Content.md` and `hackathon_docs/guide/Live-Demo-Script.md`
