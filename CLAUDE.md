...
| `Project-Description.md` | The detailed VAIC submission project description (distinct from the README and the elevator pitch) — written to convince both the Claude repo-reader and human judges. Three sections: **What we're building** (one-sentence framing + safety-first pipeline + a 6-requirement coverage table), **Who it's for** (daily outpatients/families, overloaded CSKH staff, and the hidden cardiac-emergency user), and **What stands out** (the detailed core: emergency guardrail, grounded/no-hallucination answering, Vietnamese-first multi-intent, sovereign-cloud deployability, real booking handoff, dual-reader legibility), plus an honest MVP-scope block mirroring the README's "Known simplifications." |
| `Pitch-Deck-Content.md` | Slide-by-slide narration content (English, ~5 min, 11 slides) for the pitch deck — inquiry-first (leads with grounded Q&A breadth, safety is the payoff/twist later); includes a "What It Covers" intents-coverage table (each intent → what the app does), a dedicated Technical Implementation slide alongside AI-Native Architecture (the rubric scores them separately), and a pay-as-you-go "Cost & Unit Economics" slide (metered rates, measured eval cost, hospital-scale projection). Each slide tagged with the rubric criteria it targets; ends with an all-6-criteria coverage table. Doctor as lead voice on pain/business/close. Compiled artifact: `TrustTim-Pitch-Deck.pptx`. |
| `Live-Demo-Script.md` | The ~5-min live demo video script (Vietnamese) — inquiry-first order (all 5 KB/booking topics, 2 example questions each, first fully narrated + second terse → multi-intent bonus → normal-symptom/emergency as the safety payoff, 2 questions each → out-of-scope → close), each beat with the exact query to type and what to point at on-screen, plus demo-prep tips and a cut list if running long. Uses the real working app's eval-fixture queries. |
...
- **...get the full submission project description (what/who/what-stands-out, requirement-coverage table, honest MVP scope) for VAIC Round-1 screening and judges** → `hackathon_docs/guide/Project-Description.md`
- **...get the slide-by-slide pitch deck content (English, incl. a Technical Implementation slide) or the Vietnamese live-demo script (2 questions per topic), both inquiry-first and rubric-optimized** → `hackathon_docs/guide/Pitch-Deck-Content.md` and `hackathon_docs/guide/Live-Demo-Script.md`
